...
func TestFilter_Reverse(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			slice := tc.seq.Reverse().Filter(tc.fn).Collect()
			assert.Equal(t, tc.want, slice, "Expected %v, got %v", tc.want, slice)
		})
	}
}
...
func TestFilter_Indexes_Reverse(t *testing.T) {
	t.Parallel()
	indexes := make([]int, 0)
	sequence := Values(1, 2, 3, 4, 5).Reverse().Filter(func(e int) bool { return e%2 == 1 })
	for i, e := range sequence.Fn2 {
		_ = e
		indexes = append(indexes, i)
	}
